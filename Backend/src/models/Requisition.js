...
const cleanFk = (val) => {
  if (!val || val === "" || val === "null" || val === "undefined" || val === "other" || val === "CC-ME") return null;
  return String(val).trim();
};
...
class Requisition {
  static async getAll() {
    ...
    return rows.map((r) => {
      const rCharge = cleanNum(r.rentalCharge, r.rental_charge);
      const dAdvance = cleanNum(r.depositAdvance, r.deposit_advance);
      const iCharge = cleanNum(r.installationCharge, r.installation_charge);
      const bType = r.billingType || r.billing_type || "Daily";
      const pName = r.patientName || r.patient_name || "";
      const ccId = r.care_center_id || r.careCenterId || null;
      const eqId = r.equipment_id || r.equipmentId || null;

      return {
        ...r,
        // Dono casings ensure kiye hain taaki form inputs kabhi empty na rahein
        id: r.id,
        care_center_id: ccId,
        careCenterId: ccId,
        equipment_id: eqId,
