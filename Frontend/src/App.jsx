...
function MainAppLayout({ role, handleLogout, welcomeUser, setWelcomeUser }) {
  ...
  // 🔔 3. Mark Single as Read (DB + State)
  const markNotifRead = async (id) => {
    setNotifications((prev) => 
      prev.map((n) => (n.id === id ? { ...n, read: true, is_read: 1 } : n))
    );
    try {
      await API.put(`/rental/notifications/${id}/read`);
    } catch (err) {
      console.warn("Failed to mark notification as read in DB:", err.message);
    }
  };

  // 🔔 4. Mark All as Read (DB + State)
  const markAllNotifRead = async () => {
    const unreadList = notifications.filter((n) => !n.read && n.is_read !== 1);
    setNotifications((prev) => 
    ...
    try {
      await Promise.all(unreadList.map((n) => API.put(`/rental/notifications/${n.id}/read`)));
    } catch (err) {
      console.warn("Failed to mark all as read in DB:", err.message);
    }
  };
  ...
  // 📦 6. Master & Requisitions Initial Load
  useEffect(() => {
    let isMounted = true;
  ...
  return (
    <div className="font-body flex h-screen w-full overflow-hidden bg-slate-50 relative">
      ...
      <NotificationsPanel 
        ...
        notifications={isCareCenter? notifications.filter((n) => String(n.care_center_id || n.careCenterId) === String(myCenterId)) : notifications}
