...
export default function UserProfile() {
  ...
  const isSuperAdmin = loggedUser?.role === "super_admin" || loggedUser?.role === "admin";
  const isCareCenter = loggedUser?.role === "care_center";
  const isReference = loggedUser?.role === "reference";
  const isDeliveryExec = loggedUser?.role === "delivery_executive";

  const getDisplayRole = () => {
    if (isSuperAdmin) return "Super Admin";
    if (isCareCenter) return "Care Center";
    if (isReference) return "Reference Partner (Doctor)";
    if (isDeliveryExec) return "Delivery Agent";
    return loggedUser?.role || "User";
  };

  const getDepartment = () => {
    if (isSuperAdmin) return "System Administration";
    if (isCareCenter) return "Care Center Partner";
    if (isReference) return "Medical Specialist / Clinic";
    if (isDeliveryExec) return "Logistics & Fleet";
    return "Operations";
  };

  const [profile, setProfile] = useState(() => {
    const defaultName = loggedUser?.name || loggedUser?.careCenterName || (isSuperAdmin ? "Super Admin" : "User");
    const defaultPhone = loggedUser?.phone || "";
    const defaultEmail = loggedUser?.email || (defaultPhone ? `${defaultPhone}@chikitsa.in` : "admin@chikitsa.com");
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">        
        {activeTab === "profile" && (
          <div className="space-y-6">
            <h3 className="text-sm font-bold uppercase tracking-wider text-teal-600">Personal & Facility Information</h3>
            <div className="grid gap-5 sm:grid-cols-2">
              <Field label="Full Name / Facility / Doctor Name">
                <TextInput 
                  value={profile.name} 
                  onChange={(e) => setProfile({ ...profile, name: e.target.value })} 
                />
              ...
              <Field label="Assigned Role (Read Only)">
                <TextInput 
                  readOnly 
                  value={profile.role} 
                  className="bg-slate-50 text-slate-500 cursor-not-allowed" 
          ...
          <div className="space-y-8">
            <div className="space-y-5">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-800">Change Password / Passcode</h3>
              </div>
              <div className="grid gap-4 sm:max-w-md">
                <Field label="Current Password / Passcode">
                  <TextInput 
                    type="password" 
                    placeholder="Enter current password"
                    value={passwords.current} 
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-slate-500">Accounts are authenticated and managed via the central registry.</p>
            </div>
          </div>
        )}
      </div>
