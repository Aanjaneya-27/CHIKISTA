...
//             return (
//               <button key={item.key} onClick={() => { navigate(item.key); setMobileOpen(false); }} className={`group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-teal-500/15 text-teal-300 ring-1 ring-inset ring-teal-500/30" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}>
...
//           <button onClick={() => { onOpenNotifications(); setMobileOpen(false); }} className="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-400 transition hover:bg-white/5 hover:text-white">
...
//         <button onClick={() => setMobileOpen(true)} className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-500 lg:hidden">
...
//         <button onClick={onOpenNotifications} className="relative grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-500 transition hover:bg-slate-50">
//           <Bell className="h-4 w-4" />
//           {unreadCount > 0 && <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white" />}
//         </button>

//         <div className="relative">
//           <button onClick={() => setProfileMenuOpen((v) => !v)} className="flex items-center gap-2 rounded-lg py-1 pl-1 pr-2 transition hover:bg-slate-50">
...
//           {profileMenuOpen && (
//             <div className="fade-slide-up absolute right-0 top-full z-40 mt-2 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3.5 sm:px-6">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {/* 👈 Profile Menu Container with Ref */}
              ...
              {notifications.map((n) => {
                ...
                return (
                  <div 
                    ...
                    className={`group relative flex items-start justify-between gap-3 px-5 py-4 transition hover:bg-slate-50/80 ${!n.read ? "bg-teal-50/30" : ""}`}
                  >
                    {/* Clickable Area for Mark as Read */}
                    <div 
                      onClick={() => onMarkRead && onMarkRead(n.id)} 
                      className="flex flex-1 items-start gap-3 cursor-pointer min-w-0"
                    >
